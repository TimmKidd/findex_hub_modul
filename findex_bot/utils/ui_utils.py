    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                ...
                InlineKeyboardButton(text="❌ Отклонить", callback_data=f"mod_reject:{ad_id}"),
            ]
        ]
    )
...
def rejection_reasons_kb(ad_id: int, role: str) -> InlineKeyboardMarkup:
    ...
    if role == "seeker":
        return InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="👤 Должность некорректная", callback_data=f"mod_rejr:{ad_id}:title")],
                [InlineKeyboardButton(text="🕒 График некорректный", callback_data=f"mod_rejr:{ad_id}:schedule")],
                [InlineKeyboardButton(text="💲 Зарплата некорректная", callback_data=f"mod_rejr:{ad_id}:salary")],
                [InlineKeyboardButton(text="📍 Локация некорректная", callback_data=f"mod_rejr:{ad_id}:location")],
                [InlineKeyboardButton(text="📞 Контакты некорректные", callback_data=f"mod_rejr:{ad_id}:contacts")],
                [InlineKeyboardButton(text="📝 О себе некорректно", callback_data=f"mod_rejr:{ad_id}:about")],
                [InlineKeyboardButton(text="🖼 Фото некорректное", callback_data=f"mod_rejr:{ad_id}:media")],
            ]
        )
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="📍 Локация некорректная", callback_data=f"mod_rejr:{ad_id}:location")],
            [InlineKeyboardButton(text="📞 Контакты некорректные", callback_data=f"mod_rejr:{ad_id}:contacts")],
            [InlineKeyboardButton(text="📝 Описание некорректное", callback_data=f"mod_rejr:{ad_id}:description")],
            [InlineKeyboardButton(text="🎞 Медиа некорректное", callback_data=f"mod_rejr:{ad_id}:media")],
        ]
    )
