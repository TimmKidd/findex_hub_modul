...
async def menu_diag_back(callback: CallbackQuery):
    await _send_or_edit_menu(callback)


@router.callback_query(F.data.startswith(f"{CB_DIAG_PENDING_OPEN}:"))
async def menu_diag_pending_open(callback: CallbackQuery):
...
async def menu_responds_root(callback: CallbackQuery):
    await _send_or_edit_responds_root(callback)


@router.callback_query(F.data.startswith(f"{CB_RESPONDS_ROLE}:"))
async def menu_responds_role(callback: CallbackQuery):
    parts = (callback.data or "").split(":")
    if len(parts) != 2:
        try:
            await callback.answer("⚠️ Некорректные данные", show_alert=True)
...
async def menu_responds_bucket(callback: CallbackQuery):
    parts = (callback.data or "").split(":")
    if len(parts) != 4:
        try:
            await callback.answer("⚠️ Некорректные данные", show_alert=True)
