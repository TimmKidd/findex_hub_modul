...
async def alerts_open_alias(cb: CallbackQuery):
    await alerts_menu(cb)


@router.callback_query(F.data == CB_RECHECK)
async def alerts_recheck(cb: CallbackQuery):
    await _safe_answer(cb)
    await _show_alerts_home(cb)
...
async def alerts_set_position(message: Message, state: FSMContext):
    ...
    if not raw:
        await _temp_message(message, "⚠️ Введи ключевые слова по должности.")
        return
