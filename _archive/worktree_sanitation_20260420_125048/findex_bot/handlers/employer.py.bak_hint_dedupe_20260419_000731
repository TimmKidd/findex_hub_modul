...
async def _upsert_preview(bot, chat_id: int, state: FSMContext, ad_id: int) -> None:
    ...
    final_preview_msg_id: int | None = None
    ...
    try:
        if prev_msg_id:
            if has_media:
                if prev_is_media:
                    ...
                    final_preview_msg_id = int(prev_msg_id)
                    await _cleanup_after_preview(bot, state, final_preview_msg_id, prev_msg_id)
                    return
                try:
        ...
        if has_media:
            ...
            final_preview_msg_id = int(msg.message_id)
        else:
...
async def _send_fresh_preview(bot, chat_id: int, state: FSMContext, ad_id: int) -> None:
    ...
    try:
        ...
        await _cleanup_after_preview(bot, state, final_preview_msg_id, old_msg_id)
    except Exception:
        pass
