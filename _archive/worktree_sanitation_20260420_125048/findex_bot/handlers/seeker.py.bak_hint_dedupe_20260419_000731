...
def _trash_buttons_only_notice() -> str:
    return "ℹ️ Нажми одну из кнопок выше, чтобы выбрать способ получения откликов."
...
async def _upsert_preview(bot, chat_id: int, state: FSMContext, ad_id: int) -> None:
    ...
    try:
        if prev_msg_id:
            if has_media:
                if prev_is_media:
                    ...
                    await _cleanup_after_preview(bot, state, int(prev_msg_id))
                    return
                try:
        ...
        if has_media:
            if video_id:
                msg = await bot.send_video(chat_id=chat_id, video=video_id, caption=text, reply_markup=seeker_preview_keyboard(ad_id))
            else:
                msg = await bot.send_photo(chat_id=chat_id, photo=photo_id, caption=text, reply_markup=seeker_preview_keyboard(ad_id))
            await state.update_data(**{K_PREVIEW_MSG_ID: msg.message_id, K_PREVIEW_IS_MEDIA: True})
            await _cleanup_after_preview(bot, state, int(msg.message_id))
        else:
            msg = await bot.send_message(chat_id=chat_id, text=text, reply_markup=seeker_preview_keyboard(ad_id))
...
async def _metro_close(cb: CallbackQuery):
    await safe_answer(cb)
    try:
        await _edit_metro_card(cb, _prompt_location(), reply_markup=metro_location_keyboard())
    except Exception:
        pass
...
async def _metro_pick(cb: CallbackQuery, state: FSMContext):
    ...
    try:
        await _edit_metro_card(cb, 'Выбери линию метро Москвы:', reply_markup=metro_lines_keyboard(page))
    except Exception:
        pass
...
async def _metro_line_pick(cb: CallbackQuery, state: FSMContext):
    ...
    try:
        await _edit_metro_card(cb, 'Выбери станцию метро:', reply_markup=metro_stations_keyboard(line_uid, page))
    except Exception:
        pass
...
async def _metro_station_pick(cb: CallbackQuery, state: FSMContext):
    ...
    async with get_sessionmaker()() as session:
        await AdRepo(session).patch_payload(int(ad_id), location=location_value)
    ...
    if _is_edit_mode(data):
        ...
        try:
            ...
            pass
        await _send_fresh_preview(cb.bot, cb.from_user.id, state, int(ad_id))
        return
...
async def _send_contact_mode_hint(message: Message) -> None:
    ...
    try:
        hint = await message.bot.send_message(
            ...
            text="Нажми одну из кнопок выше, чтобы выбрать способ получения откликов.",
        )
...
async def _send_photo_choice_hint(message: Message) -> None:
    ...
    try:
        hint = await message.bot.send_message(
            chat_id=message.chat.id,
            text="Нажми одну из кнопок выше: ➕ Добавить медиа или ⏭ Без медиа.",
        )
        await asyncio.sleep(4)
    except Exception:
        return
...
async def seeker_buttons_only_trash(message: Message, state: FSMContext):
    ...
    if cur == SeekerForm.preview.state:
        ...
        if (data or {}).get("on_moderation"):
            hint_text = "⏳ Объявление уже отправлено на модерацию. Дождись ответа модератора."
        else:
            hint_text = "Используй кнопки выше: исправить поле, изменить медиа или отправить объявление на модерацию."
    elif cur == SeekerForm.media_choice.state:
        hint_text = "Нажми одну из кнопок выше: ➕ Добавить медиа или ⏭ Без медиа."
    else:
        hint_text = "Используй кнопки выше."
