...
async def welcome_menu_clean_thread(message: Message, state: FSMContext):
    ...
    if await _has_active_respond_view(int(message.from_user.id)):
        hint = None
        try:
            hint = await message.bot.send_message(
                chat_id=message.chat.id,
                text=get_hint_text(RESPOND_ACTIVE_CARD_TRASH),
            )
            await asyncio.sleep(4)
        except Exception:
            return
    ...
    surface = await _get_menu_surface(int(message.from_user.id))
    hint_text = None
    if surface == "root":
